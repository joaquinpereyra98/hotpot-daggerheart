 ...
 */
export default function onRenderCharacterSheet(
  application,
  element,
  context,
  options,
  ...
  fieldsset.querySelectorAll(".inventory-item").forEach((el) => {
    el.ondragstart = /** @param {DragEvent} event */ async(event) => {
      const element = event.target.closest("[data-item-uuid]");
      const doc = await foundry.utils.fromUuid(element.dataset.itemUuid);
      if (!doc) return;
      event.dataTransfer.setData(
        "text/plain",
        JSON.stringify(doc.toDragData()),
      );
    };
  });
  ...
  if (options.isFirstRender) {
    ...
    fieldsset
      .querySelectorAll(".inventory-item-quantity")
      .forEach((el) =>
        el.addEventListener(
          "change",
          application.updateItemQuantity.bind(application),
        ),
      );
  }
}
